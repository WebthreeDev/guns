...
contract ERC721CCTNFT is ERC165, IERC721 {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(
            ...
            "ERROR: You are not the owner or dont have permissions"
        );
        _approve(to, tokenId);
    }
        ...
        override
    {
        require(
            operator != msg.sender,
            "ERROR: Operator address muts by diferent"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "ERROR: You are not owner or you don have permissions"
        );
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        require(
            _checkOnERC721Reveiver(from, to, tokenId, _data),
            "ERROR: ERC721Reveiver not implemented"
        );
    }

    uint256 public nftCommonPrice = 10000000;
    uint256 public nftEpicPrice = 20000000;
    uint256 public nftLegentadyPrice = 30000000;

    function changeNftPrice(uint256 nftType, uint256 price)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function mint(
        ...
        if (nftType == 1) nftPrice = 10000000;
        if (nftType == 2) nftPrice = 20000000;
        if (nftType == 3) nftPrice = 30000000;
    ...
    function sale() public payable returns (bool) {
        require(msg.value >= 0,"ERROR: Invalid value");
        require(msg.sender != address(0), "ERROR: Invalid zero address");
        return true;
    }

    function _checkOnERC721Reveiver(
        address from,
        address to,
        uint256 tokenId,
        bytes memory _data
        ...
        if (isContract(to)) {
            ...
            returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert(
                        "ERC721: transfer ro non ERC721Receiver implementater"
                    );
                } else {
                    assembly {
